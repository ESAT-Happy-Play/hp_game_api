from game.serializers import MagicResultSerializer, MagicResultCreateSerializer, GameScheduleSerializer, MagicResultListSerializer, MagicResultListPaginationSerializer
from .draw_result_winner import DrawResultWinnerViewSet
from game.models import MagicResult, BetItem, PrizePool, GameSchedule, CompanyGame, WinStreak
...
class MagicResultViewSet(BaseViewSet):
    queryset = MagicResult.objects.filter(isDeleted=False)
    serializer_class = MagicResultSerializer
    ...
    def list(self, request):
        ...
        serializer = MagicResultListSerializer(queryset, many=True)
    ...
    operation_id='magic_result_get_by_schedule')
    @action(detail=False, methods=['get'], url_path='(?P<gameScheduleId>[^/.]+)/game-schedule')
    def magic_result_get_by_schedule(self, request, gameScheduleId=None):
        magic_result = self.queryset.filter(gameSchedule__exact=gameScheduleId).first()

        if magic_result is None:
            return JsonResponse({'detail': 'No Magic result found for the specified game schedule ID.'}, status=status.HTTP_404_NOT_FOUND)
        
        serializer = self.serializer_class(magic_result)
    ...
    operation_id='magic_result_get_by_schedule_list')
    @action(detail=False, methods=['get'], url_path='game-schedule/list')
    def magic_result_get_by_schedule_list(self, request):
        magic_result_query = self.queryset
        
        if 'gameScheduleIds' in request.query_params:
            magic_result_query = magic_result_query.filter(gameSchedule__in=request.query_params.get('gameScheduleIds').split(','))

        serializer = self.serializer_class(magic_result_query, many=True)

        return JsonResponse(serializer.data, status=status.HTTP_200_OK, safe=False)
        
        
    @extend_schema(request=MagicResultCreateSerializer)
    ...
    @extend_schema(request=MagicResultListPaginationSerializer)
    @action(detail=False, methods=['post'], url_path='list')
    def get_paginated_magic_result_list(self, request):
        ...
        magic_results = self.queryset

        if company_id:
            magic_results = magic_results.filter(companyId=company_id)

        if start_date:
            magic_results = magic_results.filter(gameSchedule__date__gte=start_date)
